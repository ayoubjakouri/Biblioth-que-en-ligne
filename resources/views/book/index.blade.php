    ...
    <div class="py-24 bg-white">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <a href="{{ route('book.create') }}"><span class="text-blue-600 font-semibold underline">{{ __('messages.add_book') }}</span></a>
                <h2 class="mt-2 text-3xl font-extrabold text-gray-900 sm:text-4xl">{{ __('messages.browse_books') }}</h2>
            </div>
            @if(session('success'))
                            ...
                            <div class="flex justify-center gap-6 pt-3 border-t border-gray-100">
                                ...
                                <form action="{{ route('book.destroy', $book->id) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this book?')"
                                        class="group/btn flex flex-col items-center gap-1 text-gray-600 hover:text-red-600 transition cursor-pointer"
                                        title="Delete Book">
